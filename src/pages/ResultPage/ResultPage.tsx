import { OpenInNewRounded, ReplayRounded } from '@mui/icons-material';
import { Card, CardContent, Snackbar, Stack, Typography } from '@mui/material';
import { useMemo, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { EmotionCard } from '../../entities/emotion/ui/EmotionCard';
import { Track } from '../../entities/playlist/model/types';
import { PlaylistList } from '../../entities/playlist/ui/PlaylistList';
import { SavePlaylistButton } from '../../features/playlist/ui/SavePlaylistButton';
import { useMusicMoodStore } from '../../shared/model/musicStore';
import { AppButton } from '../../shared/ui/AppButton/AppButton';
import { EmptyState } from '../../shared/ui/EmptyState/EmptyState';
import { Layout } from '../../widgets/Layout/Layout';
import styles from './ResultPage.module.css';

export const ResultPage = () => {
  const navigate = useNavigate();
  const playlist = useMusicMoodStore((state) => state.currentPlaylist);
  const savedPlaylists = useMusicMoodStore((state) => state.savedPlaylists);
  const likedTracks = useMusicMoodStore((state) => state.likedTracks);
  const saveCurrentPlaylist = useMusicMoodStore((state) => state.saveCurrentPlaylist);
  const toggleLikedTrack = useMusicMoodStore((state) => state.toggleLikedTrack);
  const removeTrackFromCurrentPlaylist = useMusicMoodStore((state) => state.removeTrackFromCurrentPlaylist);
  const [snackbar, setSnackbar] = useState('');

  const likedTrackIds = useMemo(() => likedTracks.map((track) => track.id), [likedTracks]);
  const isSaved = playlist ? savedPlaylists.some((item) => item.id === playlist.id) : false;

  const handleSave = () => {
    const saved = saveCurrentPlaylist();
    setSnackbar(saved ? 'Плейлист сохранён в профиле.' : 'Плейлист уже сохранён.');
  };

  const handleToggleLike = (track: Track) => {
    const liked = toggleLikedTrack(track);
    setSnackbar(liked ? 'Трек добавлен в любимые.' : 'Трек удалён из любимых.');
  };

  if (!playlist) {
    return (
      <Layout>
        <Stack spacing={3} sx={{ width: 'min(680px, 100%)', mx: 'auto' }}>
          <EmptyState
            title="Плейлиста пока нет"
            description="Начните анализ настроения, чтобы сгенерировать плейлист для Spotify."
          />
          <AppButton onClick={() => navigate('/permissions')}>Начать подбор</AppButton>
        </Stack>
      </Layout>
    );
  }

  return (
    <Layout>
      <Stack spacing={3}>
        <div>
          <Typography variant="overline" color="primary" sx={{ fontWeight: 900 }}>
            Ваш результат
          </Typography>
          <Typography variant="h2" sx={{ fontSize: { xs: '2.2rem', md: '3.4rem' } }}>
            {playlist.name}
          </Typography>
        </div>
        <div className={styles.grid}>
          <Stack spacing={3}>
            <EmotionCard emotion={playlist.emotion} />
            <Card>
              <CardContent>
                <Stack spacing={2.5}>
                  <Typography variant="h5" sx={{ fontWeight: 850 }}>
                    Предпросмотр Spotify
                  </Typography>
                  <iframe
                    className={styles.embed}
                    title={`${playlist.name} Spotify embed`}
                    src={`https://open.spotify.com/embed/playlist/${playlist.id}`}
                    allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture"
                    loading="lazy"
                  />
                </Stack>
              </CardContent>
            </Card>
          </Stack>
          <Card>
            <CardContent>
              <Stack spacing={3}>
                <div className={styles.actions}>
                  <SavePlaylistButton disabled={isSaved} onSave={handleSave} />
                  <AppButton
                    variant="outlined"
                    startIcon={<OpenInNewRounded />}
                    onClick={() => window.open(playlist.spotifyUrl, '_blank', 'noopener,noreferrer')}
                  >
                    Открыть в Spotify
                  </AppButton>
                  <AppButton variant="text" startIcon={<ReplayRounded />} onClick={() => navigate('/permissions')}>
                    Подобрать заново
                  </AppButton>
                </div>
                <PlaylistList
                  tracks={playlist.tracks}
                  likedTrackIds={likedTrackIds}
                  onToggleLike={handleToggleLike}
                  onRemoveTrack={(trackId) => {
                    removeTrackFromCurrentPlaylist(trackId);
                    setSnackbar('Трек удалён из плейлиста.');
                  }}
                />
              </Stack>
            </CardContent>
          </Card>
        </div>
      </Stack>
      <Snackbar
        open={Boolean(snackbar)}
        autoHideDuration={2400}
        message={snackbar}
        onClose={() => setSnackbar('')}
      />
    </Layout>
  );
};
